...
def render_video_file():
    ...
    with cols[1]:
        ...
        if selected_value in list(src_video_dic.keys())[:2]:
            bShow = False
        if st.button("删除文件", disabled=not bShow):
            pass
    ...
    with cols[1]:
        ...
        if selected_value in list(des_video_dic.keys())[:1]:
            bShow = False
        if st.button("删除文件",key="del_des_file", disabled=not bShow):
            pass
    ...
    with cols[0]:
        ...
        bShow = True
        if selected_value in list(des_video_dic.keys())[:2]:
            bShow = False
        if st.button("开始处理", use_container_width=True, type="primary", disabled=bShow):
            pass

    with cols[1]:
        # 停止渲染按钮
        if st.button("删除所有源视频", use_container_width=True, type="primary"):
            pass

    with cols[2]:
        # 停止渲染按钮
        if st.button("删除所有处理后的视频", use_container_width=True, type="primary"):
            pass
